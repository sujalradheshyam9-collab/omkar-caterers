...
const OWNER_DETAILS = {
  name: 'Radheshyam Maharaj',
  phones: ['9763824571', '9579385895']
};
...
export default function OmkarCustomer({ onExitRole }) {
  ...
  const [guestOrderIds, setGuestOrderIds] = useState(() => JSON.parse(localStorage.getItem('omkar_guest_order_ids') || '[]'));
  ...
  useEffect(() => { localStorage.setItem('omkar_guest_order_ids', JSON.stringify(guestOrderIds)); }, [guestOrderIds]);
      ...
      <div className="h-screen bg-gradient-to-br from-green-500 to-green-700 flex flex-col p-4">
        <div className="bg-white rounded-2xl shadow-2xl flex-1 overflow-y-auto p-6 max-w-2xl mx-auto w-full">
          <div className="flex justify-between items-center mb-4 relative">
            <button onClick={() => setCurrentPage(isLoggedIn ? 'mainMenu' : 'home')} className="text-green-600 font-semibold">← Back</button>
            ...
            <div className="space-y-4">
              {myOrders.map((order) => (
                <div key={order.id} className={`border-2 rounded-lg p-4 ${order.status === 'accepted' ? 'bg-green-50 border-green-500' : order.status === 'cancelled' ? 'bg-gray-100 border-gray-400' : 'bg-red-50 border-red-500'}`}>
                  ...
                  {order.status === 'accepted' && <p className="text-sm"><strong>💰 ₹{(((order.pricePerGuest || 0) * (order.guestCount || 0)) + (((order.pricePerGuest || 0) * (order.guestCount || 0)) * ((order.gstPercent || 0) / 100))).toLocaleString('en-IN')}</strong> (incl. GST {order.gstPercent || 0}%)</p>}
  ...
  if (currentPage === 'viewOrderCustomer') {
    ...
    const subtotal = (order.pricePerGuest || 0) * (order.guestCount || 0);
      ...
      <div className="h-screen bg-gradient-to-br from-green-500 to-green-700 flex flex-col p-4">
        <div className="bg-white rounded-2xl shadow-2xl flex-1 overflow-y-auto p-6 max-w-2xl mx-auto w-full">
          ...
          <div className="mb-3 bg-purple-50 p-3 rounded border-l-4 border-purple-400 text-xs">
            ...
            <p><strong>Name:</strong> {OWNER_DETAILS.name}</p>
            <p className="mb-2"><strong>Phone:</strong> {OWNER_DETAILS.phones.join(' / ')}</p>
            <div className="flex gap-2">
              <a href={`tel:${OWNER_DETAILS.phones[0]}`} className="flex-1 bg-green-600 text-white font-bold py-2 rounded text-center text-xs">📞 Call {OWNER_DETAILS.phones[0]}</a>
              <a href={`tel:${OWNER_DETAILS.phones[1]}`} className="flex-1 bg-green-600 text-white font-bold py-2 rounded text-center text-xs">📞 Call {OWNER_DETAILS.phones[1]}</a>
            </div>
          </div>
      ...
      <div className="h-screen bg-gradient-to-br from-green-500 to-green-700 flex items-center justify-center p-4 overflow-y-auto">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl my-4">
          ...
          <div className="space-y-4">
            ...
            <button
              disabled={!cateringRating || !staffRating || !foodRating || !serverRating}
              onClick={async () => { await updateDoc(doc(db, 'bookings', editingBillId), { feedback: { catering: cateringRating, staff: staffRating, food: foodRating, server: serverRating, text: feedbackText } }); setCateringRating(0); setStaffRating(0); setFoodRating(0); setServerRating(0); setFeedbackText(''); alert('✅ Feedback submit!'); setCurrentPage('myOrders'); }}
              className="w-full bg-green-600 text-white font-bold py-3 rounded-lg disabled:bg-gray-300 disabled:cursor-not-allowed"
            >
              Submit ✅
            </button>
            {(!cateringRating || !staffRating || !foodRating || !serverRating) && (
              <p className="text-xs text-gray-500 text-center">सभी 4 categories में कम से कम 1 star दो</p>
            )}
          </div>
        </div>
      </div>
          ...
          <button
            onClick={() => {
              ...
              filled.forEach((d) => {
                ...
                newSelected[`${label}: ${d.text.trim()}`] = true;
              });
  ...
  if (currentPage === 'billGeneration') {
    ...
    // Order number submit hone ke baad hi generate hota hai (Firestore document ID se, hamesha unique).
    // Isliye yahan koi fake preview number nahi dikhate — confusion se bachne ke liye.
    return (
      <div className="h-screen bg-gradient-to-br from-green-500 to-green-700 flex items-center justify-center p-4 overflow-y-auto">
        <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-xl my-4">
          <div className="text-center mb-4 border-b-2 border-green-300 pb-3">
            <h1 className="text-2xl font-bold text-green-600">🍛 BILL PREVIEW</h1>
            <p className="text-xs text-gray-500 mb-1">Order number submit करने के बाद मिलेगा</p>
            <h2 className="text-lg font-bold text-green-700">OMKAR CATERERS</h2>
            <p className="text-xs text-green-600">✓ 100% PURE VEGETARIAN</p>
          </div>
          ...
          <div className="mb-3 bg-purple-50 p-3 rounded border-l-4 border-purple-400">
            ...
            <p className="text-xs"><strong>Name:</strong> {OWNER_DETAILS.name}</p>
            <p className="text-xs mb-2"><strong>Phone:</strong> {OWNER_DETAILS.phones.join(' / ')}</p>
            <div className="flex gap-2">
              <a href={`tel:${OWNER_DETAILS.phones[0]}`} className="flex-1 bg-green-600 text-white font-bold py-2 rounded text-center text-xs">📞 Call Now</a>
            </div>
          </div>
          ...
          <button onClick={async () => { try { const bill = { customerName: customerName || 'Guest', customerEmail, customerPhone, customerAddress, orderType, eventType, eventDate, eventTime, mealType, guestCount: guests, foodType, allDishes, pricePerGuest: 0, gstPercent: 0, totalAmount: 0, status: 'pending', createdAt: new Date().toLocaleString(), createdAtTs: Date.now() }; const docRef = await addDoc(collection(db, 'bookings'), bill); const finalBillId = `#OMKAR${docRef.id.slice(-6).toUpperCase()}`; await updateDoc(docRef, { billId: finalBillId }); if (!customerPhone) { setGuestOrderIds(prev => [...prev, docRef.id]); } alert('✅ Order Request भेज दिया! Owner से price का wait करें।'); setCurrentPage('myOrders'); setEventType(null); setEventDate(''); setEventTime(''); setMealType(''); setGuestCount(''); setFoodType(''); setSelectedDishes({}); setCustomDishes({}); setOwnMenuDishes({}); } catch (err) { alert('❌ Order भेजने में दिक्कत आई। Internet check करके फिर try करो।'); } }} className="w-full bg-green-600 text-white font-bold py-3 rounded-lg mb-2">✅ Send Request</button>
