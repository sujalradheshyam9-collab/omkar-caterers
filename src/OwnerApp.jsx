...
export default function OmkarOwner({ onExitRole }) {
  ...
  const [failedAttempts, setFailedAttempts] = useState(() => {
    const local = parseInt(localStorage.getItem('omkar_owner_failed_attempts') || '0');
    const session = parseInt(sessionStorage.getItem('omkar_owner_failed_attempts') || '0');
    return Math.max(local, session);
  });
  const [lockedUntil, setLockedUntil] = useState(() => {
    const local = parseInt(localStorage.getItem('omkar_owner_locked_until') || '0');
    const session = parseInt(sessionStorage.getItem('omkar_owner_locked_until') || '0');
    return Math.max(local, session);
  });
      ...
      if (!isFirstBookingsLoad.current) {
        snapshot.docChanges().forEach((change) => {
          if (change.type === 'added') {
            ...
            playNotifySound();
            setNotifications(prev => [{ id: change.doc.id + '_' + Date.now(), text: `🆕 ${b.customerName || 'Guest'} ने नया order भेजा (${b.eventDate})`, time: new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) }, ...prev].slice(0, 30));
          }
        });
      }
  ...
  useEffect(() => {
    localStorage.setItem('omkar_owner_failed_attempts', String(failedAttempts));
    sessionStorage.setItem('omkar_owner_failed_attempts', String(failedAttempts));
  }, [failedAttempts]);
  useEffect(() => {
    localStorage.setItem('omkar_owner_locked_until', String(lockedUntil));
    sessionStorage.setItem('omkar_owner_locked_until', String(lockedUntil));
  }, [lockedUntil]);
  ...
  const getSubtotal = (b) => (b?.pricePerGuest || 0) * (b?.guestCount || 0);
  ...
  const runConfirmedAction = async () => {
    ...
    try {
      ...
      alert('❌ यह action save नहीं हुआ। Internet check करके फिर try करो।');
    }
  };
  ...
  const updateBillField = async (billId, field, value) => {
    try {
      ...
      alert('❌ Update save नहीं हुआ। Internet check करके फिर try करो।');
    }
  };

  const togglePaymentStatus = (billId, current) => {
    updateBillField(billId, 'paymentStatus', current === 'paid' ? 'pending' : 'paid');
  };

  const [historyFilter, setHistoryFilter] = useState('all'); // 'all' | 'pending' | 'paid'
  ...
  const addDishToBill = async (billId) => {
    ...
    try {
      ...
      alert('❌ Dish add नहीं हुई। Internet check करके फिर try करो।');
    }
  };
  ...
  const removeDishFromBill = async (billId, index) => {
    ...
    try {
      ...
      alert('❌ Dish remove नहीं हुई। Internet check करके फिर try करो।');
    }
  };
      ...
      <div className="h-screen bg-gradient-to-br from-green-500 to-green-700 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl">
          ...
          <input
            ...
            onKeyDown={(e) => { if (e.key === 'Enter' && !locked) attemptLogin(); }}
        ...
        <div className="bg-white rounded-2xl shadow-2xl flex-1 overflow-y-auto p-6 max-w-4xl mx-auto w-full">
          ...
          <div className="grid grid-cols-7 gap-2 mb-6">
            ...
            {days.map((day, idx) => {
              ...
              // Agar ek din mein multiple bookings hain, to pending ko priority do (owner ko sabse pehle wahi dikhna chahiye),
              // uske baad accepted, uske baad cancelled — taaki koi pending order accepted ke peeche chhup na jaaye.
              const event = events.find(e => e.status === 'pending') || events.find(e => e.status === 'accepted') || events[0];
  ...
  if (currentPage === 'viewBillOwner') {
    ...
    if (!bill) return null;
    const subtotal = (bill.pricePerGuest || 0) * (bill.guestCount || 0);
  ...
  if (currentPage === 'customersList') {
    ...
    const customersSummary = uniquePhones.map(phone => {
      ...
      return { phone, name: latest.customerName || 'Unknown', orderCount: orders.length, totalSpent };
  ...
  if (currentPage === 'billHistory') {
    const sortNewestFirst = (arr) => arr.slice().sort((a, b) => (b.createdAtTs || 0) - (a.createdAtTs || 0));
    let historyBills = sortNewestFirst(bookings.filter(b => b.status === 'accepted'));
    if (historyFilter === 'pending') historyBills = historyBills.filter(b => (b.paymentStatus || 'pending') !== 'paid');
    if (historyFilter === 'paid') historyBills = historyBills.filter(b => b.paymentStatus === 'paid');
    if (historyDateSearch.trim()) historyBills = historyBills.filter(b => b.eventDate === historyDateSearch.trim());
      ...
      <div className="h-screen bg-gradient-to-br from-green-500 to-green-700 flex flex-col p-4">
        <div className="bg-white rounded-2xl shadow-2xl flex-1 overflow-y-auto p-6 max-w-2xl mx-auto w-full">
          ...
          <div className="mb-4 bg-gray-50 p-3 rounded-lg border">
            <label className="text-xs font-bold">📅 किसी particular date के orders ढूंढो:</label>
            <div className="flex gap-2 mt-1">
              <input type="date" value={historyDateSearch} onChange={(e) => setHistoryDateSearch(e.target.value)} className="flex-1 border-2 border-gray-300 rounded p-2 text-sm" />
              {historyDateSearch && <button onClick={() => setHistoryDateSearch('')} className="bg-gray-300 px-3 rounded text-sm font-bold">Clear</button>}
            </div>
          </div>

          <div className="grid grid-cols-3 gap-2 mb-4">
            <button onClick={() => setHistoryFilter('all')} className={`py-2 rounded text-xs font-bold ${historyFilter === 'all' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}>सभी</button>
            <button onClick={() => setHistoryFilter('pending')} className={`py-2 rounded text-xs font-bold ${historyFilter === 'pending' ? 'bg-orange-600 text-white' : 'bg-gray-200 text-gray-700'}`}>💰 Payment Pending</button>
            <button onClick={() => setHistoryFilter('paid')} className={`py-2 rounded text-xs font-bold ${historyFilter === 'paid' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}>✅ Payment Done</button>
          </div>

          {historyBills.length === 0 && <p className="text-center text-gray-500 mt-8">📭 कोई bill नहीं मिला</p>}

          {historyBills.map((bill) => (
            <div key={bill.id} className={`border-l-4 p-3 rounded-lg mb-3 ${bill.paymentStatus === 'paid' ? 'border-green-500 bg-green-50' : 'border-orange-400 bg-orange-50'}`}>
              ...
              <p className="text-xs text-gray-600 mb-2">{bill.eventDate} | 👥 {bill.guestCount} guests | 🍽️ {(bill.allDishes || []).length} items</p>
              <p className="text-xs font-bold text-green-600 mb-2">💰 ₹{getTotal(bill).toLocaleString('en-IN')} — {bill.paymentStatus === 'paid' ? '✅ Paid' : '⏳ Payment Pending'}</p>
              <div className="grid grid-cols-3 gap-2 text-xs">
                ...
                <button onClick={() => { setEditingBillId(bill.id); setCurrentPage('ownerEditBill'); }} className="bg-purple-600 text-white font-bold py-2 rounded">✏️ Edit</button>
                <button onClick={() => togglePaymentStatus(bill.id, bill.paymentStatus)} className={`font-bold py-2 rounded ${bill.paymentStatus === 'paid' ? 'bg-orange-500 text-white' : 'bg-green-600 text-white'}`}>{bill.paymentStatus === 'paid' ? '⏳ Mark Pending' : '✅ Mark Paid'}</button>
              </div>
  ...
  if (currentPage === 'ownerDashboard') {
    ...
    return (
      <div className="h-screen bg-gradient-to-br from-green-500 to-green-700 flex flex-col p-4">
        {renderConfirmModal()}
        <div className="bg-gradient-to-r from-green-600 to-green-800 text-white p-4 rounded-t-2xl flex justify-between items-center relative">
          <h1 className="text-2xl font-bold">👑 Owner</h1>
          <div className="flex gap-2 flex-wrap justify-end items-center">
            <button onClick={() => setShowNotifications(prev => !prev)} className="relative text-sm font-bold bg-white bg-opacity-20 px-3 py-1 rounded">
              🔔
            ...
            <button onClick={() => setCurrentPage('billHistory')} className="text-sm font-bold bg-teal-700 px-3 py-1 rounded">📜 History</button>
          ...
          {showNotifications && (
            <div className="absolute top-full right-4 mt-2 bg-white text-gray-800 rounded-xl shadow-2xl w-80 max-w-[90vw] max-h-96 overflow-y-auto z-50 border">
        ...
        <div className="bg-white rounded-b-2xl shadow-2xl flex-1 overflow-y-auto p-4 max-w-2xl mx-auto w-full">
          ...
          {pendingBills.length === 0 && cancelledBills.length === 0 && <p className="text-center text-gray-500 mt-8">📭 No pending orders</p>}
        </div>
      </div>
    );
  }
